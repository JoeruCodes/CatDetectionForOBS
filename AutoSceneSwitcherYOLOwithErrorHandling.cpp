...
int main()
{
    ...
    try
    {
        auto model = load_yolo_model();
        net = model.first;
        classes = model.second;
    }
    ...
    for (const auto &entry : scene_mapping)
    {
        if (const auto *index = std::get_if<int>(&entry.second))
        {
            try
            {
                ...
                if (!camera.isOpened())
                {
                    throw std::runtime_error("Failed to open camera " + std::to_string(*index));
                }
                cameras.push_back(std::move(camera));
            }
            catch (const std::exception &e)
            {
                std::cerr << "Error opening camera " << *index << ": " << e.what() << std::endl;
                obsws_close(ws);
                obsws_free(ws);
                exit(1);
            }
    ...
    while (true)
    {
        ...
        if (counter == 20)
        {
            ...
            {
                std::cerr << "Error switching scene: " << e.what() << std::endl;
                obsws_close(ws);
                obsws_free(ws);
                exit(1);
            }
        ...
        for (cv::VideoCapture &camera : cameras)
        {
            ...
            {
                std::cerr << "Error releasing camera frame: " << e.what() << std::endl;
                obsws_close(ws);
                obsws_free(ws);
                exit(1);
            }
